...
static void I2C_callback(uint32_t event){
	osThreadFlagsSet(id_Th_camera, event);
}

static void I2C_Init(void){
  I2C2drv->Initialize  (I2C_callback);
...
static int32_t Camera_Read (uint8_t reg, uint8_t *val) {
  ...
  data[0] = reg;
  statusMT1=I2C2drv->MasterTransmit(CAMERA_I2C_ADDR<<1, data, 1, true);
  while (I2C2drv->GetStatus().busy);
	statusGB1=I2C2drv->GetDataCount();
  if (statusGB1!= 1) return -1;
  statusMT2=I2C2drv->MasterReceive (CAMERA_I2C_ADDR<<1, val, 1, false);
  while (I2C2drv->GetStatus().busy);
...
void test_dir(void){
	uint8_t dir = 0;
	uint8_t data = 0x01;
	uint32_t event, error;

	for(dir = 0; dir < 255; dir++){
		error = I2C2drv->MasterTransmit(dir, &data, 1, true);
		event = osThreadFlagsWait(0xFFFF, osFlagsWaitAny, osWaitForever);
		printf("ADDR: %02x\t Event:%d\tError:%d\n", dir, event, error);
		osDelay(50);
	}
}
...
//	Disable_RET_PWDN();
  //HAL_TIM_PWM_Start(&htim2,TIM_CHANNEL_4); //No debe ser necesario aqui
   osDelay(1000);
    ...
    osDelay(1000); // Espera 1 segundo (ajusta el tiempo según sea necesario)

    
    result = Camera_Read(0x1C, &camera_id); 
    if (result == 0) {
        printf("Dirección I2C de la cámara: 0x%X\n", camera_id);
    } else {
        printf("Error al leer la dirección I2C de la cámara.\n");
    }
